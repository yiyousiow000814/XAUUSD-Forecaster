...
def test_gemma_edits_display_reason_without_changing_semantics(monkeypatch) -> None:
...
def test_semantic_evidence_failure_uses_exact_source_pointer_repair(
    ...
    def respond(_key, _model, payload):
        ...
        if len(calls) == 1 or _model == annotation_module.DEFAULT_GEMMA_MODEL:
            return vector
...
def test_headline_translation_does_not_fallback_after_provider_failure(
    tmp_path, monkeypatch
) -> None:
    now = datetime(2026, 8, 5, 10, 0, tzinfo=UTC)
    ledger = ForwardLedger(tmp_path / "forward.sqlite3", now=now)
    ...
    assert models == [annotation_module.DEFAULT_GEMMA_MODEL]
...
@pytest.mark.parametrize("text", [
    "iShares S&P/TSX Global Gold Index ETF 股价上涨",
    "黄金基金股价上涨",
])
def test_gemma_display_review_is_single_pass_and_preserves_semantics(monkeypatch, text):
    calls = []
    def generate(_gateway, _index, **kwargs):
        calls.append(kwargs)
        return {"headline_zh": text, "summary_zh": text,
                "primary_story_title_zh": text, "semantic_reason_zh": text,
                "xauusd_relevance": "DIRECT"}, "gemma-reviewed"
    monkeypatch.setattr(GeminiModelGateway, "generate", generate)
    original = {"headline_zh": text, "summary_zh": text,
                "primary_story_title_zh": text, "semantic_reason_zh": text,
                "xauusd_relevance": "IRRELEVANT", "supporting_evidence": ["source"]}
    expected_original = json.loads(json.dumps(original))
    original["primary_story_title_zh"] = "Original English draft"
    expected_original["primary_story_title_zh"] = "Original English draft"
    checkpoint = {"semantic_result": original, "llm_model_version": "original-semantic-model"}
    pool = annotation_module._GeminiRequestPool(("test-key",), request_accountant=ALLOW_MODEL_REQUEST)
    result, semantic_model = pool.review_display_checkpoint(
        0, checkpoint, "source", "source",
    )
    assert len(calls) == 1
    assert calls[0]["model"] == annotation_module.DEFAULT_GEMMA_MODEL
    assert calls[0]["purpose"] == "news-display-review"
    assert calls[0]["retryable_decode_errors"] == ()
    assert result["primary_story_title_zh"] == text
    assert result["xauusd_relevance"] == "IRRELEVANT"
    assert result["supporting_evidence"] == ["source"]
    assert semantic_model == "original-semantic-model"
    assert checkpoint["semantic_result"] == expected_original


def test_saved_semantics_reach_writer_through_one_gemma_review(tmp_path, monkeypatch):
    now = datetime.now(UTC)
    ledger = ForwardLedger(tmp_path / "forward.sqlite3", now=now)
    original, body = _production_shaped_identity_annotation()
    digest = hashlib.sha256(body.encode()).hexdigest()
    ledger.append_news_revision({
        "source": "review-test", "source_item_id": "saved",
        "source_published_time": now, "collector_first_seen_time": now,
        "fetched_time": now, "headline": "L.A. Law retrospective", "body": body,
        "content_hash": digest, "cluster_id": "saved",
    })
    ledger.append_annotation_display_checkpoint({
        "checkpoint_id": "saved-semantics", "source": "review-test",
        "source_item_id": "saved", "revision_number": 1,
        "raw_content_hash": digest, "llm_model_version": annotation_module.DEFAULT_GEMINI_MODEL,
        "prompt_version": annotation_module.PROMPT_VERSION,
        "semantic_result": original, "invalid_fields": ["primary_story_title_zh"],
        "rejection_reason": "retired language ratio", "captured_at": now,
    })
    before = dict(ledger.connection.execute(
        "SELECT * FROM news_annotation_display_checkpoints_v1"
    ).fetchone())
    calls = []
    def review(_key, model, _payload):
        calls.append(model)
        return {**original, "primary_story_title_zh": "L.A. Law 演员回顾"}
    _mock_model_json(monkeypatch, review)
    statuses = annotate_pending_news(
        ledger, provider="gemini", api_key="test-key", limit=1,
        request_accountant=ALLOW_MODEL_REQUEST,
    )
    assert [item["status"] for item in statuses] == ["OK"]
    assert calls == [annotation_module.DEFAULT_GEMMA_MODEL]
    stored = ledger.connection.execute("SELECT * FROM news_annotations").fetchone()
    accepted = json.loads(stored["annotation_json"])
    assert accepted["primary_story_title_zh"] == "L.A. Law 演员回顾"
    assert accepted["supporting_evidence"] == original["supporting_evidence"]
    assert stored["raw_content_hash"] == digest
