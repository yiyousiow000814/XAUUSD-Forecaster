...
SPEC = importlib.util.spec_from_file_location('source_architecture', ROOT / 'scripts/architecture/architecture_compiler.py')
compiler = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(compiler)


def test_repository_generated_architecture_is_current_in_required_python_gate():
    result = subprocess.run([sys.executable, str(ROOT / 'scripts/architecture/compile_architecture.py'), 'check'],
...
def test_build_then_check_and_tamper_are_real_cli_boundaries(request, fixture_name, monkeypatch, scoped):
    ...
    command = [sys.executable, str(source / 'scripts/architecture/compile_architecture.py')]
...
def test_mutation_kill_requires_exact_executed_assertion(tmp_path, monkeypatch, case, expected):
    ...
    spec = importlib.util.spec_from_file_location('mutation_runner', ROOT / 'scripts/architecture/run_architecture_mutations.py')
