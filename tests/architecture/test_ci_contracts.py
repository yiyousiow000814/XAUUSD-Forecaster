...
def test_windows_runtime_gate_is_parallel_bounded_and_keeps_required_name() -> None:
    ...
    assert 'test "$PLAN_RESULT" = success' in workflow
...
def test_windows_runtime_selector_uses_authoritative_impact_map(monkeypatch) -> None:
    spec = importlib.util.spec_from_file_location("select_windows_runtime_shards", ROOT / "scripts/validation/select_windows_runtime_shards.py")
...
def test_web_gate_keeps_complete_suite_and_uses_bounded_lockfile_install() -> None:
    ...
    assert "python ../scripts/validation/run_bounded_web_install.py" in workflow
    ...
    runner = (ROOT / "scripts" / "validation" / "run_bounded_web_install.py").read_text(
        encoding="utf-8"
    )
    for contract in (
        '"ci"',
...
def test_bounded_web_install_timeout_terminates_process_tree(tmp_path: Path) -> None:
    spec = importlib.util.spec_from_file_location(
        "run_bounded_web_install", ROOT / "scripts" / "validation" / "run_bounded_web_install.py"
    )
...
def test_python_shard_selector_and_runner_use_authoritative_manifest() -> None:
    selector_spec = importlib.util.spec_from_file_location(
        ...
        ROOT / "scripts" / "validation" / "select_python_test_shards.py",
    )
    ...
    runner_spec = importlib.util.spec_from_file_location(
        "run_python_test_shard", ROOT / "scripts" / "validation" / "run_python_test_shard.py",
    )
    ...
    source = (ROOT / "scripts" / "validation" / "run_python_test_shard.py").read_text(
        encoding="utf-8"
    )
    for contract in (
        '"--timeout=30"',
