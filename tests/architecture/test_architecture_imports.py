...
def checker(monkeypatch):
    monkeypatch.syspath_prepend(str(ROOT / 'scripts/architecture'))
...
def test_actual_repository_policy_and_required_workflow(checker):
    ...
    assert 'python -m pytest tests/architecture/test_architecture_imports.py -q' in workflow


def test_real_copied_cli_is_source_bound_read_only_and_never_imports_the_checked_code(source, tmp_path):
    root, policy = source
    (root / 'scripts/architecture').mkdir()
    for name in ['architecture_compiler.py', 'check_architecture_imports.py']:
        shutil.copyfile(ROOT / 'scripts/architecture' / name, root / 'scripts/architecture' / name)
    policy['script_imports'] = [{'source': 'scripts/architecture/check_architecture_imports.py',
        'target': 'scripts/architecture/architecture_compiler.py', 'reason': 'Use the shared parser.',
    ...
    other.mkdir()
    command = [sys.executable, str(root / 'scripts/architecture/check_architecture_imports.py'), '--json']
    def run():
