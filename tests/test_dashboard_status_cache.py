...
def test_status_snapshot_cache_serves_bounded_stale_during_slow_refresh(
    ...
    assert health["last_error"] == "RuntimeError: boom"

    expired_cache = StatusSnapshotCache(
        ttl_seconds=15, wait_seconds=0.01, max_stale_seconds=90,
        clock=lambda: now[0],
    )
    now[0] = 0.0
...
def test_status_snapshot_cache_health_states(tmp_path) -> None:
    now = [0.0]
    cache = StatusSnapshotCache(
    ...
    status, health = cache.health()
    assert (status, health) == (503, {
        "status": "UNAVAILABLE",
...
def test_status_snapshot_cache_waiter_times_out(tmp_path) -> None:
