...
def test_connected_worker_session_budget_reconciles_actual_manifest():
    ...
    code = "\n".join((
        ...
        "const seeded=structuredClone(plan);seeded.input_sha256.baseline_resources='d'.repeat(64);",
        "seeded.baseline_resources={regular_posts:7,market_rows:864,market_batches:36,news_records:1908,news_pages:239};",
        "seeded.phases.baseline_resources=292;seeded.maximum_commands+=292;",
        "assert.equal(sessionCommandBudget(seeded).maximum_commands,5042);",
        "for(const change of [p=>p.baseline_resources.regular_posts=8,p=>p.baseline_resources.market_rows=true,",
        "p=>p.baseline_resources.market_batches=0,p=>p.baseline_resources.news_records=10001,",
        "p=>p.baseline_resources.news_pages=0,p=>p.baseline_resources.extra=1,p=>delete p.input_sha256.baseline_resources]){",
        "const p=structuredClone(seeded);change(p);assert.throws(()=>sessionCommandBudget(p),{code:'WORKER_ADAPTER_BASELINE_RESOURCE_COUNTS_INVALID'});}",
