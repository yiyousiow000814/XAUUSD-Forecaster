...
def test_causal_quote_extract_real_gzip_and_cli_fail_closed(tmp_path):
    ...
    script = Path(__file__).resolve().parents[2]/"scripts/research/run_causal_signal_replay.py"
...
def test_real_cli_and_report_bind_inputs_and_reject_tampered_results(tmp_path):
    ...
    def run(script, *args):
        return subprocess.run([sys.executable, str(root/"scripts/research"/script), *map(str, args)],
