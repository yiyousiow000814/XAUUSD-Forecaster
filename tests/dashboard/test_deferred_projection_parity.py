...
def _module():
    path = ROOT / "scripts" / "validation" / "check_deferred_projection_parity.py"
...
def test_runtime_and_producer_roots_are_independent_cli_contracts(tmp_path) -> None:
    source = (ROOT / "scripts" / "validation" / "check_deferred_projection_parity.py").read_text(
        encoding="utf-8"
    )
...
'''
    command = [sys.executable, '-c', code, str(ROOT / 'scripts/validation/check_deferred_projection_parity.py'),
