...
def _sync_module():
    path = Path(__file__).resolve().parents[2] / "scripts" / "runtime" / "run_dashboard_sync.py"
...
def _annotator_module():
    path = Path(__file__).resolve().parents[2] / "scripts" / "runtime" / "run_news_annotator.py"
...
def _preview_module():
    path = Path(__file__).resolve().parents[2] / "scripts" / "validation" / "build_preview_bundle.py"
...
path = pathlib.Path("scripts/validation/build_preview_bundle.py").resolve()
...
except resource_contracts.PayloadContractError:
    pass
else:
    raise AssertionError("missing source detail was accepted")
"""
...
def test_audit_sync_owns_four_independently_bounded_resources(monkeypatch) -> None:
    ...
    assert [body for _url, body in writes] == expected
