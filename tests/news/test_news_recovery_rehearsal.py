...
def test_sqlite_input_identity_includes_committed_wal_and_ignores_reader_shm(tmp_path):
    import sqlite3
    spec = importlib.util.spec_from_file_location("wal_rehearsal", ROOT / "scripts/maintenance/rehearse_news_recovery_copy.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
...
def test_query_oracle_compares_real_duplicate_null_receipts(tmp_path, row_budget, monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("query_rehearsal", ROOT / "scripts/maintenance/rehearse_news_recovery_copy.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
...
def test_request_handler_must_exit_before_rehearsal_cleanup_can_pass():
    spec = importlib.util.spec_from_file_location("recovery_rehearsal", ROOT / "scripts/maintenance/rehearse_news_recovery_copy.py")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
