...
GKG candidates before retrieving publisher text. Discovery requires either an
explicit gold, bullion, or XAUUSD asset anchor in the publisher title, or a GKG
gold theme accompanied by a distinct macro or geopolitical GKG theme. This is a
positive discovery boundary, not a negative title blacklist. Gold metadata only
scopes the discovery lane; it does not decide semantic relevance or model
permission. A `PAGE_PRECISEPUBTIMESTAMP` is retained when available. Otherwise
the GKG batch
