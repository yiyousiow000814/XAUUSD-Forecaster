...
| Training and models | Requested on outcomes/reconciliation; early `NOT_DUE` | Mature eligible rows | Materialized rows and complete generation | SQLite plus immutable files | Collector THREAD | Canonical materialization, runtime, fitting and generation owners; process failure domain remains shared |
