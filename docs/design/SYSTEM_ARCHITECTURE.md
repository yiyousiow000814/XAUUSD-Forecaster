...
| Release runtime read model | `scripts/release_runtime_read_model.ps1` | Pure resolved Committed/Previous/Target identities; independently observed business health and ownership; shared WPF/WinForms presentation with one max-stale provider envelope bound to the full authority fingerprint; verified single-flight process-tree cleanup; both JSON views; and fresh Reverse action-time authority in `scripts/xauusd_control_center.ps1` | `tests/test_release_runtime_read_model.py`, `tests/test_runtime_launchers.py`, `formal/release-control/ReleaseRuntimeReadModel.tla` |
