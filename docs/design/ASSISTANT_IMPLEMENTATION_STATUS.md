...
| Local model routing | [Orchestration](../contracts/ASSISTANT_ORCHESTRATION.md) | `MVP` | Interactive chat uses one hardware-validated Qwen 3.5 4B Q4_K_M profile with a real 262K Ollama context. The 9B/8B pair was removed after full-window tests showed avoidable memory pressure and a 43 GB CPU-spilling Ministral path. The selected local model remains locked across one native tool sequence; other AI workloads retain their existing provider-specific profiles. |
