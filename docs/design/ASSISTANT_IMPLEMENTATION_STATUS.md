...
#104, #106, #108, #110, #111, #113, #114, and #116 are merged on `main`. The
rows describe the bounded Assistant foundations covered by those merged PRs
and by the current durable chat-runtime implementation.
...
| Conversation persistence | [State](../contracts/ASSISTANT_STATE.md) | `MVP` | Owner-scoped D1 Conversation and immutable Message records form provider-independent canonical history. Chat admission can append a canonical user message to an active conversation, and completion atomically appends one final Assistant message. Production activation requires migrations `0009` through `0011`; the Windows chat worker and UI remain separate scopes. |
...
| Streaming | [Behavior](../specs/ASSISTANT_BEHAVIOR.md) | `PARTIAL` | Python and TypeScript share strict `assistant.event.v1`. D1 now persists immutable contiguous turn events, and `/api/assistant-chat?mode=events` provides owner-authenticated finite SSE replay with numeric resume state. A Windows producer and browser consumer are not connected yet, so this is not end-to-end live streaming. |
| Commentary/progress | [Behavior](../specs/ASSISTANT_BEHAVIOR.md) | `PARTIAL` | The machine route accepts only validated, lease-bound, idempotent reasoning/tool/retrieval progress, with tool and retrieval operations closed inside one batch. No Windows producer or user-visible progress component exists yet. |
