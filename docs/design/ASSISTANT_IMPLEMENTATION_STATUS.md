...
| Long-term memory | [State](../contracts/ASSISTANT_STATE.md) | `PARTIAL` | D1, Vectorize, lexical/semantic union, reranking, owner scope, and point-in-time validation remain provider-neutral foundations. Production indexing is paused with Assistant and the prior local Qwen vectors were purged; a new embedding activation must declare and rebuild one complete generation. |
| Incremental compaction | [State](../contracts/ASSISTANT_STATE.md) | `PARTIAL` | Immutable summaries, pins, and leased compaction receipts remain implemented, but no worker claims compaction while Assistant has no configured model. A future API model must activate compaction and chat as one verified profile generation. |
...
| Local model routing | [Orchestration](../contracts/ASSISTANT_ORCHESTRATION.md) | `DEPRECATED` | The Qwen 3.5 4B model, installer, runtime profile, and supervised worker were removed because host memory remained unsuitable. The generic provider interface remains, but no local model is installed or advertised. |
| Multi-credential routing | [Orchestration](../contracts/ASSISTANT_ORCHESTRATION.md) | `PARTIAL` | The durable pool-by-model Capacity Router remains available and Google news workloads rank independent account pools. Assistant has no enabled model or credential route and fails closed before admission. |
...
| Unified model gateway | [Orchestration](../contracts/ASSISTANT_ORCHESTRATION.md) | `PARTIAL` | Provider-neutral metering and Google/Ollama transports remain reusable, but Assistant has no enabled gateway route. News workloads retain their separately configured providers. |
| Function calling | [Orchestration](../contracts/ASSISTANT_ORCHESTRATION.md) | `PARTIAL` | The bounded native agent and typed read-only tools remain implemented and tested, but the dedicated Assistant worker is removed and no function-calling model is active. |
