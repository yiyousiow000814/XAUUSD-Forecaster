...
#104, #106, #108, #110, #111, #113, #114, #116, and #117 are merged on `main`.
The rows describe those bounded foundations and the current Windows chat-worker
integration.
...
| Conversation persistence | [State](../contracts/ASSISTANT_STATE.md) | `MVP` | Owner-scoped D1 Conversation and immutable Message records form provider-independent canonical history. Chat admission appends a canonical user message, and the worker completion atomically appends one final Assistant message. Production activation requires migrations `0009` through `0012`; the browser UI remains a separate scope. |
...
| Streaming | [Behavior](../specs/ASSISTANT_BEHAVIOR.md) | `PARTIAL` | Python and TypeScript share strict `assistant.event.v1`. D1 persists immutable contiguous events, the Windows worker produces reasoning/tool progress, and `/api/assistant-chat?mode=events` provides owner-authenticated finite SSE replay. The responsive browser consumer is not connected yet, so this is not user-visible end-to-end streaming. |
| Commentary/progress | [Behavior](../specs/ASSISTANT_BEHAVIOR.md) | `PARTIAL` | The Windows producer publishes the deterministic public reasoning class and closed idempotent tool start/finish batches derived from exact receipts. No extra model call or private reasoning is used. A user-visible progress component remains future work. |
