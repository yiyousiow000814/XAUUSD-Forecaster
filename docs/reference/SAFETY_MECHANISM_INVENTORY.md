...
| Control Plane install | Exact-main installer; install transaction and bundle manifest | One verified bundle and supervisor; install never changes business runtime; failed install restores baseline | watchdog, guard, Sync hold, release lock | TLA+ plus `test_control_plane_install.py`; observed handoff lacked acknowledged quiescence and recovery reused a normal-state assumption |
...
| Sync ownership | Control Center and exact operation token | Zero owners only during an exact bounded hold/switch; otherwise exactly one | migration, watchdog, runtime reload, News projection | TLA+ and runtime contracts; ownership transfer must be atomic in release state |
| Coordinated migration | exact target hold and immutable migration receipt | hold cannot be defeated; stale/mismatched hold cannot suppress recovery; accepted migration evidence is preserved | Candidate identity, Sync, install, D1 | TLA+ plus migration receipt contracts; TTL requires explicit exit and owner recovery |
