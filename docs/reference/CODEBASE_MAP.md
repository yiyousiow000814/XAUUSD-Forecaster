...
| Private architecture graph | `architecture/manifest.json` | Manifest v2 purpose, lane, dependency and failure contract plus lazy read-only Explorer | `web/build/architecture-manifest.ts`, `web/app/_lib/architecture-explorer.ts` (Dagre and bounded mobile transforms), `web/app/_views/ArchitectureExplorerView.tsx` (React Flow), `web/app/_views/ArchitectureExplorerView.module.css` | `tests/test_architecture_manifest.py`, `web/tests/architecture-explorer.test.mjs` | [Architecture manifest](../../architecture/README.md), [Package dependencies](../contracts/PACKAGE_DEPENDENCIES.md), [Web and Cloudflare](../design/WEB_AND_CLOUDFLARE.md) |
