...
| Private architecture graph | `architecture/manifest.json` | Compact manifest v2 rows expanded at build time into the purpose, lane, dependency and failure contract plus lazy read-only Explorer | `web/build/architecture-manifest.ts`, `web/app/_lib/architecture-explorer.ts` (Dagre and bounded mobile transforms), `web/app/_lib/architecture-camera.ts` (single latest-intent viewport owner), `web/app/_views/ArchitectureExplorerView.tsx` (React Flow), `web/app/_views/ArchitectureExplorerView.module.css` | `tests/test_architecture_manifest.py`, `web/tests/architecture-explorer.test.mjs` | [Architecture manifest](../../architecture/README.md), [Package dependencies](../contracts/PACKAGE_DEPENDENCIES.md), [Web and Cloudflare](../design/WEB_AND_CLOUDFLARE.md) |
