...
| Private architecture graph | `architecture/manifest.json` | Manifest v2 graph contract and lazy read-only Explorer | `web/build/architecture-manifest.ts`, `web/app/_lib/architecture-explorer.ts` (Dagre transform), `web/app/_views/ArchitectureExplorerView.tsx` (React Flow), `web/app/_views/ArchitectureExplorerView.module.css` | `tests/test_architecture_manifest.py`, `web/tests/architecture-explorer.test.mjs` | [Architecture manifest](../../architecture/README.md), [Web and Cloudflare](../design/WEB_AND_CLOUDFLARE.md) |
