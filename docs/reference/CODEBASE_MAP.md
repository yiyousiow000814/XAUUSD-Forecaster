...
| Private architecture graph | `architecture/manifest.json` | Compact manifest v2 rows expanded at build time into beginner-first navigation, progressive edge disclosure, purpose, lane, dependency and failure contracts plus optional orientation-independent semantic ranks, tracks and convergences; complete-view layout stays stable while disclosure filters rendered edges and ports | `web/build/architecture-manifest.ts`, `web/app/_lib/architecture-explorer.ts` (Dagre fallback, disclosure, bounded semantic spacing and readable mobile viewport), `web/app/_lib/architecture-camera.ts` (single latest-intent viewport owner), `web/app/_views/ArchitectureExplorerView.tsx` (Explore/Reference React Flow), `web/app/_views/ArchitectureExplorerView.module.css` | `tests/test_architecture_manifest.py`, `web/tests/architecture-explorer.test.mjs` | [Architecture manifest](../../architecture/README.md), [Package dependencies](../contracts/PACKAGE_DEPENDENCIES.md), [Web and Cloudflare](../design/WEB_AND_CLOUDFLARE.md) |
