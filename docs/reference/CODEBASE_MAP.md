...
| Private architecture graph | Repository source plus `architecture/declarations/` and `architecture/contracts/`; `architecture/generated/` is derived | The offline compiler uses Python AST, the TypeScript compiler API, a certainty-labelled PowerShell fallback/Windows evidence boundary, and bounded C# inventory. It emits a compact high-level Explorer manifest plus separately bounded lazy code/evidence indexes. The private build expands learner navigation, exact evidence, dependency modes, test effectiveness, and generated module/symbol drill-down while preserving layout/disclosure/camera/mobile contracts. | `scripts/architecture_compiler.py`, `scripts/compile_architecture.py`, `web/build/extract-architecture.mjs`, `web/build/architecture-manifest.ts`, `web/build/architecture-evidence.ts`, `web/app/_lib/architecture-explorer.ts`, `web/app/_lib/architecture-evidence.ts`, `web/app/_lib/architecture-camera.ts`, `web/app/_lib/architecture-mobile-interaction.ts`, `web/app/_views/ArchitectureExplorerView.tsx`, `web/app/_views/ArchitectureEvidencePanels.tsx` | `tests/test_architecture_compiler.py`, `tests/test_architecture_manifest.py`, `web/tests/architecture-explorer.test.mjs`, `web/tests/architecture-mobile-interaction.test.mjs`, `web/tests/architecture-evidence.test.mjs` | [Generated architecture](../../architecture/README.md), [Compiler design](../design/ARCHITECTURE_EVIDENCE_COMPILER.md), [Artifact protocol](../protocols/ARCHITECTURE_ARTIFACTS.md), [Architecture evidence](../contracts/ARCHITECTURE_EVIDENCE.md), [Package dependencies](../contracts/PACKAGE_DEPENDENCIES.md) |
