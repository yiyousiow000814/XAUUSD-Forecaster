...
test("uses one modal timeline for model generations and market decisions", () => {
  ...
  assert.match(modal, /useLayoutEffect\(\(\) => \{[\s\S]*settleResponsiveScroll\(options => bodyRef\.current\?\.scrollTo\(options\), pendingScrollTop\.current!\);[\s\S]*\}, \[tab\]\)/);
...
test("keeps dashboard navigation and graph controls usable on phones", () => {
  ...
  assert.match(page, /pendingScrollTop\.current = window\.scrollY;[\s\S]*useLayoutEffect\(\(\) => \{[\s\S]*settleResponsiveScroll\(options => window\.scrollTo\(options\), pendingScrollTop\.current!\);[\s\S]*\}, \[view\]\)/);
  assert.match(dashboard, /pendingScrollTop\.current = currentScrollTop;[\s\S]*useLayoutEffect\(\(\) => \{[\s\S]*settleResponsiveScroll\(options => window\.scrollTo\(options\), pendingScrollTop\.current!\);[\s\S]*\}, \[location\]\)/);
  ...
  assert.match(responsiveScroll, /scroll\(\{ top, left: 0, behavior: "instant" \}\)/);
