...
test("separates gold news and hides explicitly irrelevant semantic reviews", () => {
  ...
  assert.match(route, /\$\{relevance\} = 'CONTEXT_ONLY'/);
  ...
  assert.match(route, /classification_complete: \(unclassifiedTotalRow\?\.count \?\? 0\) === 0/);
  assert.match(source, /scope: newsScope/);
  assert.match(source, /旧索引不会显示为“其他”/);
