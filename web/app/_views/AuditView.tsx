      ...
      {view === "evidence" && <section className="evidence-desk">
        ...
        <div className="evidence-table-wrap"><table className="evidence-table">
          ...
          <tbody>{visibleEvidence.map(row => <tr key={row.event_key}>
            <td className="evidence-status-cell"><span className={`model-seen-badge ${row.model_seen ? "is-seen" : "is-unseen"}`}>{row.model_seen ? "已用于预测" : "未用于预测"}</span><small>{EVIDENCE_LABELS[row.evidence_grade] ?? row.evidence_grade}<br />{row.model_seen ? "当时确实参与了模型输入" : row.broad_model_eligible ? "现在符合条件，等待下一次预测" : "现在也不符合使用条件"}</small></td>
