...
import {
  architectureCommitSha, architectureFailureImpact, architectureGithubHref, architectureRelations,
...
const DIMENSIONS = [
  ["ownership", "Ownership"], ["boundary", "Boundary"], ["critical_path", "Critical Path"],
  ["bounded_work", "Bounded Work"], ["incremental", "Incremental"], ["failure_isolation", "Failure Isolation"],
] as const;
...
type FlowEdgeData = Record<string, unknown> & { edge: ArchitectureEdge; highlighted: boolean; dimmed: boolean; guided: boolean };
...
const ArchitectureGraphEdge = memo(function ArchitectureGraphEdge(props: EdgeProps<ArchitectureFlowEdge>) {
  ...
  return <>
    ...
    <EdgeLabelRenderer><span className={`${styles.edgeLabel} ${data.dimmed ? styles.edgeLabelDimmed : ""}`}
...
function Inspector({ manifest, node, impact, sha, onClose, onDrill }: {
  ...
  const names = (ids: string[]) => ids.map(id => manifest.nodes.find(item => item.id === id)?.short_label).filter(Boolean).join(" · ") || "None";
  return <aside aria-labelledby="architecture-inspector-title" className={styles.inspector}>
    <header><div><span>{node.kind} · {node.runtime_state}</span><h2 id="architecture-inspector-title">{node.label}</h2></div>
      <button aria-label="Close inspector" onClick={onClose} type="button">×</button></header>
    <div className={styles.inspectorBody}>
      <dl className={styles.beginnerDetails}>
        <div><dt>What is this?</dt><dd>{node.summary}</dd></div>
        <div><dt>Why does it exist?</dt><dd>{node.architecture.ownership}</dd></div>
        <div><dt>What enters?</dt><dd>{names(relations.directUpstream)}</dd></div>
        <div><dt>What leaves?</dt><dd>{names(relations.directDownstream)}</dd></div>
        <div><dt>If it fails, what stops?</dt><dd>{impact?.affected.map(item => item.message).join(" ") ?? node.architecture.failure_isolation}</dd></div>
        <div><dt>What continues?</dt><dd>{impact?.continues.map(item => item.message).join(" ") ?? "No unaffected path is inferred; inspect the owned failure contract below."}</dd></div>
      </dl>
      {node.subsystem_view ? <button className={styles.drillButton} onClick={() => onDrill(node.id)} type="button">Open subsystem <span aria-hidden="true">→</span></button> : null}
...
function ExplorerGraph({ manifest }: { manifest: ArchitectureManifest }) {
  ...
  const changeView = useCallback((next: string, remember = false) => {
    ...
    setViewId(next); setSelectedId(null); setHoveredId(null); setFailureMode(false);
    window.requestAnimationFrame(() => flow.fitView({ padding: mobile ? 0.14 : 0.1, duration: 320 }));
  }, [flow, mobile, viewId]);
  const selectNode = useCallback((id: string) => { setSelectedId(id); setFailureMode(false); }, []);
  ...
  useEffect(() => {
    const close = (event: KeyboardEvent) => { if (event.key === "Escape") { setSelectedId(null); setFailureMode(false); } };
    window.addEventListener("keydown", close); return () => window.removeEventListener("keydown", close);
  }, []);
  useEffect(() => {
    if (!nodesInitialized) return;
    const focus = scenario && !scenario.failure_node_id ? scenario.node_ids[scenarioStep] : selectedId;
    const item = focus ? graph.nodes.find(node => node.id === focus) : null;
    const timeout = window.setTimeout(() => item
      ? flow.setCenter(item.position.x + item.width / 2, item.position.y + item.height / 2, { zoom: mobile ? .9 : 1, duration: 300 })
      : flow.fitView({ padding: mobile ? 0.14 : 0.1, maxZoom: 1, duration: 260 }), 80);
    return () => window.clearTimeout(timeout);
  }, [flow, graph.direction, graph.nodes, graph.view.id, mobile, nodesInitialized, scenario, scenarioStep, selectedId]);
  ...
  const flowEdges: ArchitectureFlowEdge[] = graph.edges.map(item => ({
    ...
    animated: scenarioEdges.has(item.id), data: { edge: item, highlighted: highlightedEdges.has(item.id), dimmed: hasFocus && !highlightedEdges.has(item.id), guided: scenarioEdges.has(item.id) },
  }));
  ...
  const runScenario = (id: string) => {
    ...
    if (next) {
      ...
      if (next.failure_node_id) { setSelectedId(next.failure_node_id); setFailureMode(true); }
        ...
        if (target) window.setTimeout(() => flow.setCenter(target.position.x + target.width / 2, target.position.y + target.height / 2, { zoom: mobile ? .9 : 1, duration: 300 }), 420);
      }
    }
  };
  const moveScenario = (direction: number) => {
    ...
    if (target) window.setTimeout(() => flow.setCenter(target.position.x + target.width / 2, target.position.y + target.height / 2, { zoom: mobile ? .9 : 1, duration: 260 }), 80);
  };
  ...
  return <main className={styles.main}>
    <header className={styles.header}>
      <div><span>PRIVATE · BUILD {sha?.slice(0, 8) ?? "UNVERIFIED"}</span><h1>System Architecture</h1><p>{graph.view.summary}</p></div>
    ...
    <section className={styles.toolbar} aria-label="Architecture graph toolbar">
      <label className={styles.search}><span>Search owner, file, test, tag, or change target</span><input aria-label="Search architecture" onChange={event => setQuery(event.currentTarget.value)} placeholder="training, release, dashboard, retry…" type="search" value={query} />
      ...
      <button aria-pressed={failureMode} className={styles.failureButton} disabled={!impactForSelected} onClick={() => setFailureMode(value => !value)} type="button">故障影响</button>
      <button className={styles.fitButton} onClick={() => flow.fitView({ padding: mobile ? 0.14 : 0.1, duration: 280 })} type="button">Fit</button>
    </section>
    ...
    <section className={`${styles.stage} ${selected ? styles.withInspector : ""}`}>
      <div className={styles.canvas} data-graph-direction={graph.direction} data-testid="architecture-graph">
        <ReactFlow<ArchitectureFlowNode, ArchitectureFlowEdge> nodes={flowNodes} edges={flowEdges} nodeTypes={nodeTypes} edgeTypes={edgeTypes}
          ...
          {!mobile ? <MiniMap aria-label="Architecture minimap" pannable zoomable nodeColor={node => (node.data.node as ArchitectureNode).runtime_state === "PAUSED" ? "#a88b55" : "#137d74"} /> : null}
        </ReactFlow>
      ...
      {selected ? <Inspector impact={activeImpact} manifest={manifest} node={selected} onClose={() => { setSelectedId(null); setFailureMode(false); }} onDrill={drill} sha={sha} /> : null}
    </section>
    ...
    <details className={styles.textFallback}><summary>Relationship text fallback</summary><div>{graph.edges.map(edge => <p key={edge.id}><b>{manifest.nodes.find(node => node.id === edge.from)?.short_label}</b><span>{edge.label} · {edge.kind} · {edge.criticality}</span><b>{manifest.nodes.find(node => node.id === edge.to)?.short_label}</b></p>)}</div></details>
