...
function MarketChart({ market, identity, setIdentity }: { market?: MarketData; identity: string; setIdentity: (value: string) => void }) {
  ...
  return <div className="chart-block market-chart-block">
    ...
    <div className="market-controls" aria-label="K线图显示控制">
      ...
      <button className={showTraining ? "active" : ""} type="button" disabled={!versionMarkers.length} aria-pressed={showTraining} onClick={() => setShowTraining(value => !value)}>模型换版本 {formatExactCount(versionMarkers.length)}</button>
    ...
    <svg className="learning-svg" viewBox="0 0 960 400" role="img" aria-label="XAUUSD K线与模型决策">
      ...
      {showTraining && versionMarkers.map(row => <g key={`${row.decision_time}-${row.model_version}`}><title>{`${timeLabel(row.decision_time)} · 图中首次使用版本 ${row.model_version} · 前一版本 ${row.previous_model_version}`}</title><line x1={xTime(row.decision_time)} x2={xTime(row.decision_time)} y1="52" y2="332" className="training-line" /><text x={xTime(row.decision_time)+4} y="328" className="training-label">换至 {row.model_version.slice(0, 6)}</text></g>)}
