...
def main():
    ...
    lines.extend(["", "Daily, original-version, generation and fold contributions are retained in results.json/ledgers and the frozen input join. No broker fills exist; recorded effective actions remain WAIT. Closed drawdown excludes mark-to-market and is incomplete if exposure cannot be settled.",
        ...
        "Complete commands and source/input SHA256 are in execution.json. Re-run scripts/research/run_causal_signal_replay.py with its recorded command and a NEW output directory, then scripts/research/report_causal_signal_replay.py --run <output> --frozen <b501da8a-results>. The quote inputs are frozen copies; no production reads are required for replay. all_scenario_costs.csv contains every delay x frozen-cost stress; no favorable combination was selected.",
